...
static void do_xmp_profile (const struct ddr3_xmp_profile * profile, const char * name, const double mtb) {
  ...
  min_tras = profile->min_tras_lsb + ((int) (profile->min_tras_trc_upper_nibble & 15) << 8);
...
void do_ddr3 (const struct ddr3_sdram_spd * eeprom) {
  ...
  /* voltage */
  linebuf[0] = 0;
  if (eeprom->voltage & 4) addlist (linebuf, "1.2V");
  if (eeprom->voltage & 2) addlist (linebuf, "1.35V");
  if (!(eeprom->voltage & 1)) addlist (linebuf, "1.5V");
